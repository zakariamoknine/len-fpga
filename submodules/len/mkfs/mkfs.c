...
main(int argc, char *argv[])
{
	...
	for(i = 2; i < argc; i++){
		...
		shortname = slash ? slash + 1 : argv[i];
...
iappend(uint32_t inum, void *xp, int n)
{
	...
	while(n > 0){
		...
		if(fbn < NDIRECT){
			...
			// Double-indirect
			uint32_t dbn  = fbn - NDIRECT - NINDIRECT;
			uint32_t idx1 = dbn / NINDIRECT;
			uint32_t idx2 = dbn % NINDIRECT;
			...
			rsect(xint(dindirect[idx1]), (char*)indirect);
			if(indirect[idx2] == 0){
				indirect[idx2] = xint(freeblock++);
				wsect(xint(dindirect[idx1]), (char*)indirect);
			}
